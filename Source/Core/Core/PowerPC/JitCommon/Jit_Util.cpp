...
bool EmuCodeBlock::UnsafeLoadToReg(X64Reg reg_value, OpArg opAddress, int accessSize, s32 offset, bool signExtend, MovInfo* info)
{
...
void EmuCodeBlock::SafeLoadToReg(X64Reg reg_value, const Gen::OpArg & opAddress, int accessSize, s32 offset, BitSet32 registersInUse, bool signExtend, int flags)
{
	...
	if (jit->jo.fastmem && !(flags & SAFE_LOADSTORE_NO_FASTMEM) && !slowmem)
	{
		...
		MovInfo mov;
		bool offsetAddedToAddress = UnsafeLoadToReg(reg_value, opAddress, accessSize, offset, signExtend, &mov);
...
void EmuCodeBlock::SafeLoadToRegImmediate(X64Reg reg_value, u32 address, int accessSize, BitSet32 registersInUse, bool signExtend)
{
	...
	switch (accessSize)
	{
	case 64: ABI_CallFunctionC(reinterpret_cast<void*>(&PowerPC::Read_U64), address); break;
	case 32: ABI_CallFunctionC(reinterpret_cast<void*>(&PowerPC::Read_U32), address); break;
	case 16: ABI_CallFunctionC(reinterpret_cast<void*>(&PowerPC::Read_U16_ZX), address); break;
	case 8:  ABI_CallFunctionC(reinterpret_cast<void*>(&PowerPC::Read_U8_ZX), address); break;
	}
...
static OpArg FixImmediate(int accessSize, OpArg arg)
{
	if (arg.IsImm())
	{
		arg = accessSize == 8  ? arg.AsImm8() :
		      accessSize == 16 ? arg.AsImm16() :
		                         arg.AsImm32();
	}
	return arg;
}
...
void EmuCodeBlock::ForceSinglePrecision(X64Reg output, const OpArg& input, bool packed, bool duplicate)
{
	...
	{
		// input == output, only need to deal with the duplicate case
		if (duplicate)
			MOVDDUP(output, R(output));
	}
}
...
void EmuCodeBlock::SetFPRF(Gen::X64Reg xmm, Gen::X64Reg scratch)
{
	...
	if (cpu_info.bSSE4_1)
	{
		...
		SHR(64, R(scratch), Imm8(63)); // Get the sign bit; almost all the branches need it.
		...
		LEA(32, scratch, MScaled(scratch, MathUtil::PPC_FPCLASS_NN - MathUtil::PPC_FPCLASS_PN, MathUtil::PPC_FPCLASS_PN));
		continue1 = J();
		...
		MOV(32, R(scratch), Imm32(MathUtil::PPC_FPCLASS_QNAN));
		continue2 = J();

		// Max exponent + no mantissa: sign ? PPC_FPCLASS_NINF : PPC_FPCLASS_PINF;
		SetJumpTarget(notNAN);
		LEA(32, scratch, MScaled(scratch, MathUtil::PPC_FPCLASS_NINF - MathUtil::PPC_FPCLASS_PINF, MathUtil::PPC_FPCLASS_PINF));
		continue3 = J();
		...
		LEA(32, scratch, MScaled(scratch, MathUtil::PPC_FPCLASS_ND - MathUtil::PPC_FPCLASS_PD, MathUtil::PPC_FPCLASS_PD));
		continue4 = J();

		// Zero: sign ? PPC_FPCLASS_NZ : PPC_FPCLASS_PZ;
		SetJumpTarget(zero);
		SHL(32, R(scratch), Imm8(4));
		ADD(32, R(scratch), Imm8(MathUtil::PPC_FPCLASS_PZ));
	}
	else
	{
		MOVQ_xmm(R(scratch), xmm);
		TEST(64, R(scratch), M(psDoubleExp));
		FixupBranch zeroExponent = J_CC(CC_Z);
		AND(64, R(scratch), M(psDoubleNoSign));
		CMP(64, R(scratch), M(psDoubleExp));
		FixupBranch nan = J_CC(CC_G); // This works because if the sign bit is set, scratch is negative
		FixupBranch infinity = J_CC(CC_E);
		MOVQ_xmm(R(scratch), xmm);
		SHR(64, R(scratch), Imm8(63));
		LEA(32, scratch, MScaled(scratch, MathUtil::PPC_FPCLASS_NN - MathUtil::PPC_FPCLASS_PN, MathUtil::PPC_FPCLASS_PN));
		continue1 = J();
		SetJumpTarget(nan);
		MOVQ_xmm(R(scratch), xmm);
	...
	SHL(32, R(scratch), Imm8(FPRF_SHIFT));
	OR(32, PPCSTATE(fpscr), R(scratch));
}
