...
void Jit64::cmpXX(UGeckoInstruction inst)
{
	...
	else
	{
		...
		{
			if (ra.IsImm())
				MOV(64, R(input), Imm32(ra.SImm32()));
			else
		...
		else
		{
			if (ra.IsImm())
			{
				MOV(32, R(input), Imm32(ra.Imm32()));
			}
			else if (comparand.IsZero())
			{
...
void Jit64::mulli(UGeckoInstruction inst)
{
	...
	{
		auto xd = rd.Bind(d == a ? Jit64Reg::ReadWrite : Jit64Reg::Write);
		...
		else
		{
			...
			else
			{
				...
				if (((u64)(magic+1) * (max_quotient*divisor-1)) >> (shift + 32) != max_quotient-1)
				{
					...
					if (d != a)
						MOV(32, xd, ra);
					IMUL(64, xd, R(RSCRATCH));
					ADD(64, xd, R(RSCRATCH));
...
void Jit64::rlwinmx(UGeckoInstruction inst)
{
	...
	else
	{
		...
		{
			MOVZX(32, mask_size, xa, ExtractFromReg(s, inst.SH ? (32 - inst.SH) >> 3 : 0));
			needs_sext = false;
		}
