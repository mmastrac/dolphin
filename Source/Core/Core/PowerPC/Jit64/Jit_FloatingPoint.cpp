...
void Jit64::HandleNaNs(UGeckoInstruction inst, X64Reg xmm_out, X64Reg xmm)
{
	...
	std::array<FPURegister, 3> allInputs{{ regs.fpu.Lock(a), regs.fpu.Lock(b), regs.fpu.Lock(c) }};
	...
	{
		auto& r = *it;
		if (!js.op->fregsIn[r.PPCRegister()])
			continue;
		if (foundRegs.find(r.PPCRegister()) != foundRegs.end())
			continue;

		foundRegs.insert(r.PPCRegister());
		inputs.emplace(inputs.end(), r);
	}
	if (inst.OPCD != 4)
	{
		// not paired-single
