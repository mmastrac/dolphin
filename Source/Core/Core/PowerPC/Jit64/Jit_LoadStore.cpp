...
void Jit64::lXXx(UGeckoInstruction inst)
{
	...
	auto xd = rd.Bind(Jit64Reg::Write);
	if (byte_reversed)
		SafeLoadSwap(xd, ra, rb, accessSize, signExtend, update);
	else
		SafeLoad(xd, ra, rb, accessSize, signExtend, update);
}
...
void Jit64::stXx(UGeckoInstruction inst)
{
	...
	if (byte_reverse)
		SafeWriteSwap(rs, ra, rb, accessSize, update);
	else
		SafeWrite(rs, ra, rb, accessSize, update);
}
