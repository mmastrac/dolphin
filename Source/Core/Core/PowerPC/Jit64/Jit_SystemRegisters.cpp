...
void Jit64::mtmsr(UGeckoInstruction inst)
{
	...
	WriteExitDestInRSCRATCH(scratch);
}
...
void Jit64::mcrf(UGeckoInstruction inst)
{
	...
	{
		MOV(64, scratch, PPCSTATE(cr_val[inst.CRFS]));
...
void Jit64::mcrxr(UGeckoInstruction inst)
{
	...
	MOVZX(32, 8, scratch, PPCSTATE(xer_ca));
	MOVZX(32, 8, scratch2, PPCSTATE(xer_so_ov));
	// [0 SO OV CA]
	LEA(32, scratch, MComplex(scratch, scratch2, SCALE_2, 0));
	// [SO OV CA 0] << 3
	SHL(32, scratch, Imm8(4));

	MOV(64, scratch, MDisp(scratch, (u32)(u64)m_crTable));
	MOV(64, PPCSTATE(cr_val[inst.CRFD]), scratch);
...
void Jit64::mffsx(UGeckoInstruction inst)
{
	...
	AND(32, scratch, Imm32(~0x60000000));

	// FPSCR.VX = (FPSCR.Hex & FPSCR_VX_ANY) != 0;
	XOR(32, scratch2, scratch2);
	TEST(32, scratch, Imm32(FPSCR_VX_ANY));
	SETcc(CC_NZ, scratch2);
	SHL(32, scratch2, Imm8(31 - 2));
	...
	auto xd = rd.Bind(BindMode::Write);
	MOV(64, scratch2, Imm64(0xFFF8000000000000));
	OR(64, scratch, scratch2);
...
// Needs value of FPSCR in a register
void Jit64::UpdateMXCSR(GPRNative& fpscr)
{
	auto scratch = regs.gpr.Borrow();
	LEA(64, scratch, M(&s_fpscr_to_mxcsr));
	AND(32, fpscr, Imm32(7));
