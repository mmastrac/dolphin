...
void Any<T>::Lock()
{
	...
	case RegisterType::Bind:
	{
		...
		case BindMode::Reuse:
			...
			m_data.xreg = Location().GetSimpleReg();
			break;
		case BindMode::Read:
			m_data.xreg = m_reg->BindToRegister(m_data.reg, true, false);
			break;
		case BindMode::ReadWrite:
			m_data.xreg = m_reg->BindToRegister(m_data.reg, true, true);
			break;
		case BindMode::Write:
		case BindMode::WriteTransaction:
			m_data.xreg = m_reg->BindToRegister(m_data.reg, false, true);
			break;
		default:
			_assert_msg_(REGCACHE, 0, "Unhandled bind %d", m_data.mode);
			break;
		}
		auto& xreg = m_reg->m_xregs[m_data.xreg];
		_assert_msg_(DYNAREC, xreg.ppcReg == m_data.reg, "Attempted to bind register bound elsewhere");
		xreg.locked++;
		break;
	}
	case RegisterType::Borrow:
	{
		if (m_data.xreg == INVALID_REG)
		{
			// This should happen when the lock is realized to give us the best chance at choosing unused regs
			m_data.xreg = m_reg->GetFreeXReg(true);
			DEBUG_LOG(REGCACHE, "Borrowing register %d (auto)", m_data.xreg);
		}
		else
		{
			auto& xreg = m_reg->m_xregs[m_data.xreg];
		...
		auto& xreg = m_reg->m_xregs[m_data.xreg];
		_assert_msg_(DYNAREC, xreg.ppcReg == INVALID_REG, "Attempted to borrow locked register");
	...
	case RegisterType::PPC:
	{
		auto& ppc = m_reg->m_regs[m_data.reg];
		ppc.locked++;
		DEBUG_LOG(REGCACHE, "++ Lock %zu (count = %d)", m_data.reg, ppc.locked);
		break;
	}
	}
}
...
void Any<T>::Unlock()
{
	...
	case RegisterType::Borrow:
	{
		auto& xreg = this->m_reg->m_xregs[m_data.xreg];
		xreg.locked--;
	...
	case RegisterType::PPC: 
	{
		auto& ppc = m_reg->m_regs[m_data.reg];
		ppc.locked--;
		DEBUG_LOG(REGCACHE, "-- Unlock %zu (count = %d)", m_data.reg, ppc.locked);
...
template<Type T>
Native<T> RegisterClassBase<T>::Borrow(X64Reg which)
{
	RegisterData data = { .type = RegisterType::Borrow, .xreg = which };
...
void RegisterClassBase<T>::Flush()
{
	...
	for (auto& xreg : m_xregs)
	{
		...
		_assert_msg_(REGCACHE, xreg.locked == 0 || xreg.ppcReg != INVALID_REG, "Someone forgot to unlock borrowed X64 reg %u", i);
	}
...
X64Reg RegisterClassBase<T>::GetFreeXReg(bool scratch)
{
	if (scratch)
	{
		...
		for (X64Reg xr : GetScratchAllocationOrder())
			if (m_xregs[xr].locked == 0 && m_xregs[xr].ppcReg == INVALID_REG)
				return xr;
	}
	...
	for (X64Reg xreg : GetAllocationOrder())
	{
		...
		if (m_xregs[xreg].locked > 0 && m_xregs[xreg].ppcReg == INVALID_REG)
			continue;
