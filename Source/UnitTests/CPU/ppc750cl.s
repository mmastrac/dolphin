...
# mode tests with fspr) there are multiple copies of the same instruction
...
2: bl 1b
   ...
   # bcctr (unconditional)
0: bl 2f               # Get the address of the next instruction so we can
1: addi %r3,%r3,0f-1b  # compute the address of label 0f for bctr.
   mtctr %r3
...
2: .int 0x00000000              #   0(%r30): 0.0f
   ...
   .int 0x3FD55555              #  12(%r30): 1.666...f
   .int 0x7FD00000              #  16(%r30): QNaN
   .int 0xFFA00000              #  20(%r30): SNaN
   .int 0x3FAAAAAB              #  24(%r30): 1.333...f (rounded toward nearest)
   .int 0x3FC00000              #  28(%r30): 1.5f
   .int 0x40000001              #  32(%r30): 2.0f + 1ulp
   .int 0xB3800000              #  36(%r30): 2.0f - (1.333...f * 1.5f)
   .int 0x00800000              #  40(%r30): minimum single-precision normal
   .balign 8
...
0: lfd %f3,328(%r31)        # huge * normal + -normal
   ...
   lfd %f4,344(%r31)
   bl check_fpu_pnorm
0: lfs %f3,40(%r30)         # minimum_normal * minimum_normal + -minimum_normal
   fneg %f4,%f3
   fmadds %f3,%f3,%f3,%f4
   bl record
   lfs %f4,40(%r30)
...
0: li %r0,0
   ...
   addi %r6,%r6,32
0: li %r3,1
   stwcx. %r3,0,%r4  # stwcx. to the wrong address should still clear the resv.
   stwcx. %r8,%r4,%r7
   bl record
   li %r3,-1
   beq 1f
...
.if TEST_PAIRED_SINGLE
   ...
   # particularly problematic for testing because the lfd instruction
   # (which normally stores an FPR's bit pattern directly to memory)
   # causes a switch out of paired-single mode, and there are no store
   # store instructions for paired-single mode which operate on the
   # entire register, so we have no way to see exactly what's going on.
   #
...
0: li %r0,0
   ...
   lfs %f6,16(%r30)    # (QNaN,QNaN)
   lfs %f7,20(%r30)    # (SNaN,SNaN)
...
0: mtfsf 255,%f0
   psq_l %f3,16(%r30),0,0  # SNaN in ps1
   bl record
   psq_st %r3,0(%r4),0,0
   lwz %r3,0(%r4)
   lis %r7,0x7FD0
   ...
   beq 0f
1: addi %r6,%r6,32
0: psq_l %f3,20(%r30),1,0  # SNaN in ps0
   bl record
   psq_st %r3,0(%r4),0,0
   lwz %r3,0(%r4)
   lis %r7,0xFFA0
